...
/*
 * 
 * This file has been heavily edited and modified for use with CharcoalEngine, a product of Retrofire Studios
 * 
 */

namespace CharcoalEngine.Editing
{
    public class GizmoComponent
    {
        ...
        private List<Transform> _selectionPool = null;
        ...
        /// <summary>
        /// Sets a pool of Transform-objects for the gizmo.
        /// </summary>
        /// <param name="selectables"></param>
        public void SetSelectionPool(List<Transform> selectables, Transform parent)
        {
            _selectionPool = selectables;
            Parent = parent;
        }

        public IEnumerable<Transform> GetSelectionPool()
        {
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (Selection.Count > 0)
            {
                ...
                if (WasButtonHeld(MouseButtons.Right) && ActiveAxis != GizmoAxis.None)
                {
                    switch (ActiveMode)
                    {
                        ...
                        case GizmoMode.CenterTranslate:
                            {
                                ...
                                if (ActiveMode == GizmoMode.Translate || ActiveMode == GizmoMode.CenterTranslate)
                                {
                        ...
                        case GizmoMode.Rotate:
                            {
                                ...
                                delta = mousePosition.X - _lastMouseState.X;
                ...
                if (WasButtonHeld(MouseButtons.Right) /*&& !IsAnyModifierPressed()*/)
                {
                        ...
                        if (ActiveMode == GizmoMode.Translate)
                        {
                            foreach (var entity in Selection)
                            {
                                if (entity.Parent != null)
                                    entity.Position += Vector3.Transform(_translationDelta, Matrix.Invert(entity.Parent.AbsoluteYawPitchRoll));
                                else
                                    entity.Position += _translationDelta;
                            }
                        }
                        if (ActiveMode == GizmoMode.CenterTranslate)
                        {
                            foreach (var entity in Selection)
                            {
                                if (entity.Parent != null)
                                    entity.Center += Vector3.Transform(_translationDelta, Matrix.Invert(entity.Parent.AbsoluteYawPitchRoll));
                                else
                                    entity.Center += _translationDelta;
                            }
                        }
                    ...
                    if (_rotationDelta != Matrix.Identity)
                    {
                        foreach (var entity in Selection)
                        {
                            Vector3 vdelta = new Vector3(0, 0, 0);
                            switch (ActiveAxis)
                            {
                                case GizmoAxis.X:
                                    vdelta = new Vector3(0, delta, 0);
                                    break;
                                case GizmoAxis.Y:
                                    vdelta = new Vector3(delta, 0, 0);
                                    break;
                                case GizmoAxis.Z:
                                    vdelta = new Vector3(0, 0, -delta);
                                    break;
                            }
                            OnRotateEvent(entity, _rotationDelta, vdelta);
                        }
        ...
        {
            _position = Selection[0].AbsolutePosition;
            ...
            _gizmoWorld = _screenScaleMatrix;
            //if (ActiveMode == GizmoMode.CenterTranslate)
                _gizmoWorld = _gizmoWorld * Matrix.CreateTranslation(Selection[0].Center);
            _gizmoWorld = _gizmoWorld * Selection[0].AbsoluteWorld;
        ...
        protected void PickObject(Vector2 mousePosition, bool removeFromSelection)
        {
            ...
            Transform obj = null;
            foreach (Transform entity in _selectionPool)
            {
                //Console.WriteLine("hi");
                float? intersection = entity.Select(ray);
                if (intersection.HasValue && intersection < closest)
                {
                    if (entity.AbsoluteBoundingBox.Contains(Camera.Position) != ContainmentType.Contains)
                    {
        ...
        private Vector3 GetSelectionCenter()
        {
            ...
            foreach (var selected in Selection)
                center += selected.AbsolutePosition;
        ...
        public void Draw()
        {
            ...
            //_gizmoWorld = _screenScaleMatrix * Selection[0].AbsoluteWorld;
            ...
            for (int i = 0; i < 3; i++)
            {
                Vector3 screenPos =
                  _graphics.Viewport.Project(_modelLocalSpace[i].Translation + _modelLocalSpace[i].Backward + _axisTextOffset,
                                             Camera.Projection, Camera.View, _gizmoWorld);
        ...
        {
            return "Mode: " + ActiveMode + " | Snapping:" +
                   (SnapEnabled ? "ON" : "OFF") +
        ...
        private void CreateSelectionBox()
        {
            ...
            {
                BoundingBox boundingBox = transformable.AbsoluteBoundingBox;
                Vector3[] boundingBoxCorners = boundingBox.GetCorners();
                for (int i = 0; i < boundingBoxCorners.Length; i++)
                {
                    boundingBoxCorners[i] -= transformable.AbsolutePosition;
                }
        ...
        private void DrawSelectionBox()
        {
            ...
            if (Selection.Count > 0)
            {
                _selectionBoxEffect.View = Camera.View;
                _selectionBoxEffect.Projection = Camera.Projection;
                _selectionBoxEffect.World = Selection[0].AbsoluteWorld;
        ...
        private void OnTranslateEvent(Transform transformable, Vector3 delta)
        {
            if (TranslateEvent != null)
                TranslateEvent(transformable, new TransformationEventArgs(delta), new TransformationEventArgs(0));
        }

        private void OnRotateEvent(Transform transformable, Matrix delta, Vector3 adelta)
        {
            if (RotateEvent != null)
                RotateEvent(transformable, new TransformationEventArgs(delta), new TransformationEventArgs(adelta));
        }

        private void OnScaleEvent(Transform transformable, Vector3 delta)
        {
            if (ScaleEvent != null)
                ScaleEvent(transformable, new TransformationEventArgs(delta), new TransformationEventArgs(0));
        }
        ...
        /// </summary>
        public void RotationHelper(Transform entity, TransformationEventArgs e, TransformationEventArgs d)
        {
            ...
            entity.YawPitchRoll += (Vector3)d.Value;
    ...
    public delegate void TransformationEventHandler(Transform transformable, TransformationEventArgs e, TransformationEventArgs d);
