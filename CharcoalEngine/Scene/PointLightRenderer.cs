...
namespace CharcoalEngine.Scene
{
    ...
    class PointLightRenderer : DrawingSystem
    {
        ...
        public PointLightRenderer(Viewport v)
        {
            viewport = v;
        ...
        public void Draw(RenderTarget2D Normal, RenderTarget2D Depth, RenderTarget2D Diffuse)
        {
            ...
            Vector3[] light_positions = new Vector3[lights.Count];
            Vector3[] light_colors = new Vector3[lights.Count];
            float[] light_specular_powers = new float[lights.Count];
            float[] light_radii = new float[lights.Count];
            float[] light_specular_intensities = new float[lights.Count];
            for (int i = 0; i < lights.Count; i++)
            {
                light_positions[i] = lights[i].AbsolutePosition;
                light_colors[i] = lights[i].Color;
                light_specular_powers[i] = lights[i].SpecularPower;
                light_radii[i] = lights[i].Radius;
                light_specular_intensities[i] = lights[i].SpecularIntensity;
            }

            effect.Parameters["LightPosition"].SetValue(light_positions);
            effect.Parameters["LightColor"].SetValue(light_colors);
            effect.Parameters["SpecularPower"].SetValue(light_specular_powers);
            effect.Parameters["SpecularIntensity"].SetValue(light_specular_intensities);
            effect.Parameters["LightAttenuation"].SetValue(light_radii);
